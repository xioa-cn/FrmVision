...
public enum ContentCommunicationType
{
    TCPSERVICE,
    TCPCLIENT,
    MODBUSTCP,
...
public sealed class ContentCommunicationConfiguration
{
    ...
    [Category("TCP"), DisplayName("连接超时 (ms)")]
    ...
    [Category("文本"), DisplayName("文本编码"), TypeConverter(typeof(CommunicationEncodingConverter)),
     Description("必须与发送端一致：UTF-8 选 utf-8；GBK/GB2312 选 gb2312；UTF-16 小端选 unicode。先停止通讯，修改保存后再启动。二进制数据请查看 HEX。")]
    public string EncodingName { get; set; } = "utf-8";

    [Category("文本"), DisplayName("接收日志输出"),
    ...
    [Category("串口"), DisplayName("接收空闲 (ms)"), Description("连续字节间隔不超过该时间时合并为一次接收。USB 串口常把同一帧拆成多次到达，默认 20。0 表示立即回调。")]
    public int ReceiveIdleMilliseconds { get; set; } = 20;

    [Category("Modbus"), DisplayName("站号")]
    ...
    [JsonIgnore]
    public string Endpoint => Type == ContentCommunicationType.MODBUSRTU || Type == ContentCommunicationType.SERIALPORT
        ? SerialPort + " / " + BaudRate
        : (Type == ContentCommunicationType.MODBUSTCP ? "0.0.0.0" : Host) + ":" + Port;
    ...
    public static string GetTypeText(ContentCommunicationType type)
    {
        switch (type)
        {
            ...
            case ContentCommunicationType.UDPCLIENT: return "UDP 客户端";
            default: return "未知协议";
        }
    }
}
...
public sealed class ContentCommunicationViewModel : IDisposable
{
    ...
    public BindingList<ContentCommunicationConfiguration> Configurations { get; } =
        new BindingList<ContentCommunicationConfiguration>();
    ...
    public ContentCommunicationViewModel(string configurationFilePath = null)
    {
        ...
        try
        {
            ...
            var items =
                JsonConvert.DeserializeObject<List<ContentCommunicationConfiguration>>(
        ...
        {
            ConfigurationLoadError = "内部通讯配置加载失败：" + ex.Message;
        }
    }
    ...
    public Result SaveConfiguration(ContentCommunicationConfiguration draft)
    {
        lock (_sync)
        {
            try
            {
                ...
                copy.Host = copy.Host?.Trim();
                copy.SerialPort = copy.SerialPort?.Trim();
                var items = Configurations.Select(c => c.Clone()).ToList();
                int index = items.FindIndex(c => c.Id == copy.Id);
                if (index < 0) items.Add(copy);
                else items[index] = copy;
                ValidateAll(items);
                Persist(items);
                if (index < 0) Configurations.Add(copy);
                else Configurations[index] = copy;
    ...
    public static string ValidateConfiguration(ContentCommunicationConfiguration c)
    {
        ...
        bool udp = c.Type == ContentCommunicationType.UDPSERVICE || c.Type == ContentCommunicationType.UDPCLIENT;
        ...
        if (tcp || udp)
        {
            ...
            if ((c.Type == ContentCommunicationType.TCPSERVICE || c.Type == ContentCommunicationType.UDPSERVICE) && !IPAddress.TryParse(c.Host.Trim(), out _))
        ...
        if (tcp || udp || serial)
        {
            try
            {
                Encoding.GetEncoding(c.EncodingName ?? "");
            }
        ...
        if (modbus)
        {
            ...
            if (!Enum.TryParse(c.DataFormat, out DataFormat format) || !Enum.IsDefined(typeof(DataFormat), format))
                return "数据格式必须为 ABCD、BADC、CDAB 或 DCBA。";
        }

        if (serial || c.Type == ContentCommunicationType.MODBUSRTU)
        {
        ...
        if (serial && (c.ReceiveIdleMilliseconds < 0 || c.ReceiveIdleMilliseconds > 5000))
            return "接收空闲时间范围必须为 0–5000 ms。";
    ...
    private void Persist(IList<ContentCommunicationConfiguration> items)
    {
        if (ConfigurationLoadError != null)
            throw new InvalidOperationException(ConfigurationLoadError + " 请先修复或备份并移走配置文件，然后重新启动软件。");
    ...
    public async Task<Result> RestoreStartedCommunicationsAsync()
    {
        ...
        lock (_sync)
        {
            ...
            ids = Configurations.Where(c => c.AutoStart)
                .OrderBy(c => c.Type == ContentCommunicationType.TCPCLIENT || c.Type == ContentCommunicationType.UDPCLIENT ? 1 : 0)
    ...
    private async Task<Result> StartCoreAsync(string id, bool restoring)
    {
        ...
        finally
        {
            lock (_sync)
            {
                if (id != null) _starting.Remove(id);
            }
    ...
    private IDisposable CreateRuntime(ContentCommunicationConfiguration c)
    {
        ...
        switch (c.Type)
        {
            case ContentCommunicationType.TCPSERVICE:
                tcp = new ContentTcpServer(c.Port, c.Host, Encoding.GetEncoding(c.EncodingName)); break;
            case ContentCommunicationType.TCPCLIENT:
                tcp = new ContentTcpClient(c.Host, c.Port, Encoding.GetEncoding(c.EncodingName), c.ConnectTimeout,
                    c.AutoReconnect, c.ReconnectInterval); break;
            case ContentCommunicationType.SERIALPORT:
                ...
                break;
            case ContentCommunicationType.UDPCLIENT:
                tcp = new ContentUdpClient(c.Host, c.Port, Encoding.GetEncoding(c.EncodingName));
                break;
            ...
            case ContentCommunicationType.MODBUSTCP:
                return new ContentModbusTcpServer(c.Port, (byte)c.Station,
                    (DataFormat)Enum.Parse(typeof(DataFormat), c.DataFormat));
    ...
    public async Task<Result> SendAsync(string id, string text)
    {
        try
        {
            ...
            {
                EnsureAvailable();
                tcp = _runtimes.TryGetValue(id, out var runtime) ? runtime as IContentServer : null;
            }

            if (tcp == null) return Result.Fail("请先启动通讯。");
    ...
    public bool HasRuntime(string id)
    {
        lock (_sync) return id != null && _runtimes.ContainsKey(id);
    }
    ...
    public ContentTcpServer ResolveTcpServer(string key)
    {
        lock (_sync)
        {
            ...
            var configuration =
                Configurations.FirstOrDefault(c => string.Equals(c.Id, normalized, StringComparison.OrdinalIgnoreCase))
            ...
            if (!_runtimes.TryGetValue(configuration.Id, out var runtime) || !(runtime is ContentTcpServer server) ||
                !server.IsStarted)
    ...
    public ContentTcpClient ResolveTcpClient(string key)
    {
        lock (_sync)
        {
            ...
            if (!_runtimes.TryGetValue(configuration.Id, out var runtime) || !(runtime is ContentTcpClient client) ||
                !client.IsRunning)
    ...
    public ContentModbusTcpServer ResolveModbusTcpServer(string key)
    {
        lock (_sync)
        {
            ...
            if (!_runtimes.TryGetValue(configuration.Id, out var runtime) ||
                !(runtime is ContentModbusTcpServer server) || !server.IsStarted)
    ...
    public ContentModbusRtuServer ResolveModbusRtuServer(string key)
    {
        lock (_sync)
        {
            ...
            if (!_runtimes.TryGetValue(configuration.Id, out var runtime) ||
                !(runtime is ContentModbusRtuServer server) || !server.IsStarted)
    ...
    public OriginalSerialPort ResolveSerialPort(string key)
    {
        lock (_sync)
        {
            EnsureAvailable();
            string normalized = (key ?? string.Empty).Trim();
            if (normalized.Length == 0) throw new InvalidOperationException("原始串口名称不能为空。");
            var configuration =
                Configurations.FirstOrDefault(c => string.Equals(c.Id, normalized, StringComparison.OrdinalIgnoreCase))
                ?? Configurations.FirstOrDefault(c =>
                    string.Equals(c.Name, normalized, StringComparison.OrdinalIgnoreCase));
            if (configuration == null) throw new InvalidOperationException("未找到内置通讯配置：" + normalized + "。");
            if (configuration.Type != ContentCommunicationType.SERIALPORT)
                throw new InvalidOperationException("通讯“" + configuration.Name + "”不是原始串口。");
            if (!_runtimes.TryGetValue(configuration.Id, out var runtime) || !(runtime is OriginalSerialPort port) ||
                !port.IsStarted)
                throw new InvalidOperationException("原始串口“" + configuration.Name + "”未打开，请先启动内置通讯。");
            return port;
        }
    }
    ...
    public ContentUdpClient ResolveUdpClient(string key)
    {
        lock (_sync)
        {
            EnsureAvailable();
            string normalized = (key ?? string.Empty).Trim();
            if (normalized.Length == 0) throw new InvalidOperationException("UDP 客户端名称不能为空。");
            var configuration = Configurations.FirstOrDefault(c => string.Equals(c.Id, normalized, StringComparison.OrdinalIgnoreCase))
                ?? Configurations.FirstOrDefault(c => string.Equals(c.Name, normalized, StringComparison.OrdinalIgnoreCase));
            if (configuration == null) throw new InvalidOperationException("未找到内置通讯配置：" + normalized + "。");
            if (configuration.Type != ContentCommunicationType.UDPCLIENT)
                throw new InvalidOperationException("通讯“" + configuration.Name + "”不是 UDP 客户端。");
            if (!_runtimes.TryGetValue(configuration.Id, out var runtime) || !(runtime is ContentUdpClient server) || !server.IsStarted)
                throw new InvalidOperationException("UDP 客户端“" + configuration.Name + "”未启动，请先启动内置通讯。");
            return server;
        }
    }
    public string GetStatus(string id)
    {
        lock (_sync)
        {
            ...
            if (runtime is ContentUdpClient) return "已启动 / UDP 对端状态未知";
            if (runtime is OriginalSerialPort) return "已打开";
            return runtime is ContentTcpClient ? "已连接" : "运行中";
        }
    }

    private static bool IsStarted(IDisposable runtime) =>
        runtime is IContentServer tcp ? tcp.IsStarted : ((IContentNodeServer)runtime).IsStarted;
    ...
    {
        if (_disposed) throw new ObjectDisposedException(nameof(ContentCommunicationViewModel));
    }
