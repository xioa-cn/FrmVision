...
{
    public enum UdpClientOperationMode { Read, Write }
    public enum UdpClientWriteValueSource { Input, FixedValue }

    [STNode("内置通讯", "xioa", "1327916255@qq.com", "https://github.com/xioa-cn/",
        "读取内置 UDP 客户端收到的数据报并传递，或向配置的远端发送输入值 / 固定文本。")]
    // 作者：xioa
    // 作者邮箱：1327916255@qq.com
    public sealed class CoCommUdpCliNode : WorkflowNode, IEditorExecutableNode, IEditorNodeReadiness
    {
        public CoCommUdpCliNode()
        {
            SetNodeTypeTitle("UDP客户端");
        ...
        [STNodeProperty("通讯名称", "内置通讯中的 UDP 客户端名称或配置 ID，例如 UDP1；使用已经启动的客户端。")]
        public string CommunicationKey { get; set; } = string.Empty;

        [STNodeProperty("操作模式", "Read：输入触发后一直等待接收数据报并输出，可停止流程取消等待；Write：向配置的远端发送文本。")]
        public UdpClientOperationMode OperationMode { get; set; } = UdpClientOperationMode.Read;

        [STNodeProperty("写入取值方式", "Input：将上游输入值转换为文本发送；FixedValue：发送固定文本，输入口仅用于触发。")]
        public UdpClientWriteValueSource ValueSource { get; set; } = UdpClientWriteValueSource.Input;
        ...
        public EditorNodeExecutionResult Execute(EditorExecutionContext context)
        {
            ...
            try
            {
                if (!Enum.IsDefined(typeof(UdpClientOperationMode), OperationMode))
                    throw new InvalidOperationException("UDP 操作模式无效。");
                var server = context.ResolveUdpClient(CommunicationKey);
                string text;
                if (OperationMode == UdpClientOperationMode.Read)
                {
                    text = server.ReadMessage(0, context.CancellationToken);
                }
                else
                {
                    if (!Enum.IsDefined(typeof(UdpClientWriteValueSource), ValueSource))
                        throw new InvalidOperationException("UDP 写入取值方式无效。");
                    object value = ValueSource == UdpClientWriteValueSource.Input ? Input.Data : FixedValue;
                ...
                Output.Data = text;
                Output.TransferData();
                return EditorNodeExecutionResult.Success("UDP 客户端“" + CommunicationKey.Trim() + "”" +
                    (OperationMode == UdpClientOperationMode.Read ? "读取" : "发送") + "成功，已传递文本。", Output);
            }
        ...
        public EditorNodeReadinessResult CanExecute(EditorExecutionContext context)
        {
            ...
            if (OperationMode == UdpClientOperationMode.Write && ValueSource == UdpClientWriteValueSource.Input && Input.Data == null)
