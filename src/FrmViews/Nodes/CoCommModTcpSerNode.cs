namespace FrmViews.Nodes
{
    ...
    public enum ModbusServerOperationMode { Read, Write }
    public enum ModbusServerWriteValueSource { Input, FixedValue }
    public enum ModbusServerValueType { Bool, Short, UShort, Int, UInt, Long, ULong, Float, Double, String }
    ...
    public sealed class CoCommModTcpSerNode : WorkflowNode, IEditorExecutableNode, IEditorNodeReadiness
    {
        ...
        {
            SetNodeTypeTitle("Modbus TCP服务器");
        ...
        [STNodeProperty("通讯名称", "内置通讯中已启动的 Modbus TCP 从站名称或配置 ID；复用其数据区、站号和数据格式。")]
        ...
        public EditorNodeExecutionResult Execute(EditorExecutionContext context)
        {
            ...
            try
            {
                ...
                var server = context.ResolveModbusTcpServer(CommunicationKey);
                object value;
                switch (ValueType)
                {
                ...
                Output.Data = value;
                Output.TransferData();
                return EditorNodeExecutionResult.Success("Modbus TCP 服务端“" + CommunicationKey.Trim() + "”地址 " + Address.Trim() +
        ...
        private object ExecuteTyped<T>(ContentModbusTcpServer server)
        {
        ...
        private string ExecuteString(ContentModbusTcpServer server)
        {
            if (OperationMode == ModbusServerOperationMode.Read)
            {
