...
namespace ST.Library.UI.NodeEditor
{
    public abstract class STNode
    {
        ...
        private Color _RuntimeHighlightColor = Color.Empty;
        /// <summary>
        /// 获取或设置仅用于运行时显示的节点高亮颜色，不参与节点保存。
        /// </summary>
        [System.ComponentModel.Browsable(false)]
        public Color RuntimeHighlightColor {
            get { return _RuntimeHighlightColor; }
            set {
                if (value == _RuntimeHighlightColor) return;
        ...
        public string RuntimeText {
            ...
            set {
                ...
                if (text == _RuntimeText) return;
                _RuntimeText = text;
                this.Invalidate(new Rectangle(-5, -5, Math.Max(this._Width, 180) + 10,
                    this._Height + 34));
            }
        }

        private Color _RuntimeTextColor = Color.FromArgb(59, 130, 246);
        /// <summary>
        /// 获取或设置仅用于运行时值显示的状态颜色，不参与节点保存。
        /// </summary>
        [System.ComponentModel.Browsable(false)]
        public Color RuntimeTextColor {
            get { return _RuntimeTextColor; }
            set {
                if (value == _RuntimeTextColor) return;
        ...
        protected internal virtual bool IsPropertyVisible(string propertyName) { return true; }
        /// <summary>
        /// 绘制整个Node
        /// </summary>
        ...
        protected internal virtual void OnDrawNode(DrawingTools dt) {
            ...
            this.OnDrawTitle(dt);
            this.OnDrawBody(dt);
            this.OnDrawRuntimeHighlight(dt);
            this.OnDrawRuntimeText(dt);
        }

        private void OnDrawRuntimeHighlight(DrawingTools dt) {
            if (this._RuntimeHighlightColor.IsEmpty) return;
            ...
            brush.Color = Color.FromArgb(88, this._RuntimeHighlightColor.R,
                this._RuntimeHighlightColor.G, this._RuntimeHighlightColor.B);
            g.FillRectangle(brush, this.Rectangle);

            g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.None;
            pen.Color = Color.FromArgb(255, this._RuntimeHighlightColor.R,
                this._RuntimeHighlightColor.G, this._RuntimeHighlightColor.B);
            pen.Width = 4;
            g.DrawRectangle(pen, this._Left + 1, this._Top + 1,
                Math.Max(1, this._Width - 3), Math.Max(1, this._Height - 3));

            pen.Color = oldPenColor;
            pen.Width = oldPenWidth;
        }

        private void OnDrawRuntimeText(DrawingTools dt) {
            if (string.IsNullOrEmpty(this._RuntimeText)) return;
            ...
            Color oldPenColor = pen.Color;
            float oldPenWidth = pen.Width;
            Rectangle rect = new Rectangle(this._Left, this.Bottom + 4,
                Math.Max(this._Width, 180), 20);

            g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.None;
            brush.Color = Color.FromArgb(238, 35, 39, 47);
            g.FillRectangle(brush, rect);
            pen.Color = this._RuntimeTextColor;
            pen.Width = 1;
            g.DrawRectangle(pen, rect.X, rect.Y, Math.Max(1, rect.Width - 1),
                Math.Max(1, rect.Height - 1));

            Rectangle textRect = new Rectangle(rect.X + 5, rect.Y + 1,
                Math.Max(1, rect.Width - 10), Math.Max(1, rect.Height - 2));
            m_sf.Alignment = StringAlignment.Near;
            m_sf.LineAlignment = StringAlignment.Center;
            m_sf.Trimming = StringTrimming.EllipsisCharacter;
            brush.Color = Color.White;
            g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.HighQuality;
            g.DrawString(this._RuntimeText, this._Font, brush, textRect, m_sf);
