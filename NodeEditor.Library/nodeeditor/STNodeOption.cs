...
namespace ST.Library.UI.NodeEditor
{
    public class STNodeOption
    {
        ...
        public object Data {
            ...
            set {
                if (value != null) {
                    ...
                    var t = value.GetType();
                    if (!this._DataType.IsAssignableFrom(t)) {
                        throw new ArgumentException("无效数据类型 数据类型必须兼容指定的数据类型");
                    }
                }
                _Data = value;
            }
        ...
        public virtual ConnectionStatus CanConnect(STNodeOption op) {
            ...
            if (this._IsInput && !this._DataType.IsAssignableFrom(op._DataType)) return ConnectionStatus.ErrorType;
        ...
        public List<STNodeOption> GetConnectedOption() {
            ...
            foreach (var v in this._Owner.Owner.GetConnectionInfo()) {
                if (!this._IsInput && v.Output == this) lst.Add(v.Input);
                else if (this._IsInput && v.Input == this) lst.Add(v.Output);
            }
            return lst;
        }
        /// <summary>
